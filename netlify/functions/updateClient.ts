...
export const handler: Handler = async (event, context) => {
  console.log('Netlify Function Triggered: updateClient');
  ...
  try {
    ...
    console.log(`[NETLIFY INFO] Updating campaign profile for client "${clientData.name}" with slug: ${slug}`);
    ...
    console.log(`[NETLIFY SUCCESS] Client profile slug: ${slug} was successfully updated.`);
    return {
      statusCode: 200,
      headers: {
    ...
    console.error('[NETLIFY ERROR] updateClient function failed:', err);
    return {
      statusCode: 500,
      headers: {
        'Content-Type': 'application/json',
